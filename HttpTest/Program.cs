    ...
    class Program
    {
        ...
        private static void Result(IAsyncResult ar)
        {
            ...
            else
            {
                ...
                string requestStr = request.RawUrl;
                string[] str = requestStr.Split('?');
                if (str.Length > 1 && str[0].Contains("files.txt"))
                {

                    returnByteArr = File.ReadAllBytes("Resource/files.txt");
                    //returnObj = HandleClientFileListUpdate();
                    //returnByteArr = Encoding.UTF8.GetBytes(returnObj);//设置客户端返回信息的编码
                }
                else if (str.Length > 1 && str[0].Contains("version.txt"))
                {
                    returnByteArr = File.ReadAllBytes("Resource/version.txt");
                }
                else
                {

                    string[] url = str[0].Split('/');
                    string fileName = url[url.Length - 1];
                    Console.WriteLine("str : " + str[0]);
                    var dataFile = File.ReadAllBytes("Resource/" + str[0]);
                    //returnObj = dataFile;
                    returnByteArr = dataFile;
                }
